...
def set_param_fn(config):
    ...
    config.parameters.Report_Event_Recorder_Events = ["Births", "NewInfectionEvent", "VaccineDistributed2"]
    config.parameters["Listed_Events"] = ["VaccineDistributed2"]  # old school
...
def add_vax_intervention(campaign, values):
    ...
    ria = tv.new_routine_immunization(camp,
                                      efficacy=1,
                                      constant_period=30000,
                                      start_day=year_to_days(CAMP_START_YEAR) + 1,
                                      coverage=values['coverage'],
                                      co_event="VaccineDistributed2"
                                      )
    camp.add(ria)
    return {'coverage': values['coverage']}


class TestVaxRolloutCoverage(IntegrationTest):
    def setUp(self):
    ...
    def test_vax_rollout_coverage(self):
